...
import org.kuali.continuity.plan.dao.CriticalFunctionDAO.SortBy;
...
public class CriticalFunctionDAOTest extends DomainObjectTest {

	private CriticalFunctionDAO dao;
	
	public void setCriticalFunctionDAO(CriticalFunctionDAO dao) {
		this.dao = dao;
	}
	...
	private void doTestCreate() throws Exception {
		// get list
		List<CriticalFunction> dObjs = this.dao.getListByOwnerId(PlanTestData.known_plan);
	...
	private void doTestCreateDependency() throws Exception {
		...
		// create
		this.dao.createDependency(id, dObj);
		
		// assert
		dObjs = this.dao.getDependencies(id);
		this.doTestList(dObjs);
		assertEquals(++size, dObjs.size());
	}
	...
	private void doTestReadConsequences() throws Exception {
		...
		// get consequences
		Map<Consequence.Type, Consequence> dObjs = this.dao.getConsequences(id);
	...
	private void doTestReadCopingMethod() throws Exception {
		...
		// get coping methods
		CopingMethod dObj = this.dao.getCopingMethod(id);
		assertNotNull(dObj);
	}
	...
	private void doTestReadDependencies() throws Exception {
		...
		// get dependencies
		Set<Dependency> dObjs = this.dao.getDependencies(id);
		this.doTestList(dObjs);
	}
	...
	private void doTestUpdateConsequences() throws Exception {
		...
		// update
		this.dao.updateConsequences(id, dObjs);
		
		// assert
		dObjs = this.dao.getConsequences(id);
	...
	private void doTestUpdateCopingMethods() throws Exception {
		...
		// update
		this.dao.updateCopingMethod(id, dObj);
		
		// assert
		dObj = this.dao.getCopingMethod(id);
		assertNotNull(dObj);
	...
	private void doTestUpdateDescriptions() throws Exception {
		...
		// update
		String test = this.getRandomTestString();
		this.dao.updateDescriptions(id, test, test, test, test);
	...
	private void doTestUpdatePeakPeriods() throws Exception {
		...
		this.dao.updatePeakPeriods(id, dObjs, test);
		
		// assert
		dObjs = this.dao.getById(id).getPeakMonths();
		assertNotNull(dObjs);
	...
	private void doTestDeleteDependency() throws Exception {
		...
		// delete
		this.dao.deleteDependency(id, dObjs.iterator().next());
	...
	private CriticalFunction getTestCriticalFunction() throws Exception {
		List<CriticalFunction> cfs = this.dao.getSortedSubListByOwnerId(PlanTestData.known_plan, 0, 1, SortBy.id, false);
		assertNotNull(cfs);
	...
	public void testGetNonDeferrableSubListByOwnerId() throws Exception {
		List<CriticalFunction> dObjs = this.dao.getNonDeferrableSubListByOwnerId(PlanTestData.known_plan, 2074);
		assertNotNull(dObjs);
