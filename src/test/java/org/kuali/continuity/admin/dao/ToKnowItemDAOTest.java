...
import org.kuali.continuity.admin.domain.ToKnowItem.SortBy;

public class ToKnowItemDAOTest extends AdminDAOTest {
	
	private GenericAdminDAOFactory factory;
	private GenericAdminDAO<ToKnowItem> dao;
	
	@SuppressWarnings("unchecked")
	public void setFactory(GenericAdminDAOFactory factory) {
		this.factory = factory;
		this.dao = (GenericAdminDAO<ToKnowItem>)
			factory.getGenericAdminDAO(ToKnowItem.class);
	}
	...
	private void doTestCreate() throws Exception {
		ToKnowItem dObj = new ToKnowItem();
		dObj.setName("TEST");
	...
	private void doTestRead() throws Exception {
		// test 1
		ToKnowItem dObj = this.getLatest();
		ToKnowItem dSameObj = this.dao.getById(dObj.getId());
	...
	private void doTestUpdate() throws Exception {
		...
		// test
		ToKnowItem modObj = this.dao.getById(dObj.getId());
	...
	private void doTestDelete() throws Exception {
		...
		// test
		ToKnowItem nullObj = this.dao.getById(dObj.getId());
		assertNull(nullObj);
	}
	
	private ToKnowItem getLatest() throws Exception {
		List<ToKnowItem> dObjs = this.dao.getSortedListBySystemDomainId(PlanTestData.wellknown_institution, SortBy.id, false);
		return dObjs.get(0);
	}
