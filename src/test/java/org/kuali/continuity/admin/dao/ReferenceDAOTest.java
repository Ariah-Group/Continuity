...
import org.kuali.continuity.admin.domain.Reference.SortBy;

public class ReferenceDAOTest extends AdminDAOTest {
	
	private GenericAdminDAOFactory factory;
	private GenericAdminDAO<Reference> dao;
	
	@SuppressWarnings("unchecked")
	public void setFactory(GenericAdminDAOFactory factory) {
		this.factory = factory;
		this.dao = (GenericAdminDAO<Reference>)
			factory.getGenericAdminDAO(Reference.class);
	}
	...
	private void doTestRead() throws Exception {
		// test 1
		Reference dObj = this.getLatest();
		Reference dSameObj = this.dao.getById(dObj.getId());
	...
	private void doTestUpdate() throws Exception {
		...
		// test
		Reference modObj = this.dao.getById(dObj.getId());
	...
	private void doTestDelete() throws Exception {
		...
		// test
		Reference nullObj = this.dao.getById(dObj.getId());
		assertNull(nullObj);
	}
	
	private Reference getLatest() throws Exception {
		List<Reference> dObjs = this.dao.getSortedListBySystemDomainId(PlanTestData.wellknown_institution, SortBy.id, false);
		return dObjs.get(0);
	}
