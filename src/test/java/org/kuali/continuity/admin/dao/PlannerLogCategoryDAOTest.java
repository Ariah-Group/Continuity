...
import org.kuali.continuity.admin.domain.PlannerLogCategory.SortBy;

public class PlannerLogCategoryDAOTest extends AdminDAOTest {
	
	private GenericAdminDAOFactory factory;
	private GenericAdminDAO<PlannerLogCategory> dao;
	
	@SuppressWarnings("unchecked")
	public void setFactory(GenericAdminDAOFactory factory) {
		this.factory = factory;
		this.dao = (GenericAdminDAO<PlannerLogCategory>)
			factory.getGenericAdminDAO(PlannerLogCategory.class);
	}
	...
	private void doTestCreate() throws Exception {
		PlannerLogCategory dObj = new PlannerLogCategory();
		dObj.setName("TEST");
	...
	private void doTestRead() throws Exception {
		// test
		PlannerLogCategory dObj = this.getLatest();
		PlannerLogCategory dSameObj = this.dao.getById(dObj.getId());
		...
		// test 3
		dObj = this.dao.getBySystemDomainIdAndName(PlanTestData.wellknown_institution, "To record annual review session");
		assertNotNull(dObj);
	...
	private void doTestUpdate() throws Exception {
		...
		// test
		PlannerLogCategory modObj = this.dao.getById(dObj.getId());
	...
	private void doTestDelete() throws Exception {
		...
		// test
		PlannerLogCategory nullObj = this.dao.getById(dObj.getId());
		assertNull(nullObj);
	}
	
	private PlannerLogCategory getLatest() throws Exception {
		List<PlannerLogCategory> dObjs = this.dao.getSortedListBySystemDomainId(PlanTestData.wellknown_institution, SortBy.id, false);
		return dObjs.get(0);
	}
