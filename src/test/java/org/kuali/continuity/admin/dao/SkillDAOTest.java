...
import org.kuali.continuity.admin.domain.Skill.SortBy;

public class SkillDAOTest extends AdminDAOTest {
	
	private GenericAdminDAOFactory factory;
	private GenericAdminDAO<Skill> dao;
	
	@SuppressWarnings("unchecked")
	public void setFactory(GenericAdminDAOFactory factory) {
		this.factory = factory;
		this.dao = (GenericAdminDAO<Skill>)
			factory.getGenericAdminDAO(Skill.class);
	}
	...
	private void doTestCreate() throws Exception {
		...
		dObj.setDescription("Some Description");
	...
	private void doTestRead() throws Exception {
		// test 1
		Skill dObj = this.getLatest();
		Skill dSameObj = this.dao.getById(dObj.getId());
	...
	private void doTestUpdate() throws Exception {
		...
		// test
		Skill modObj = this.dao.getById(dObj.getId());
	...
	private void doTestDelete() throws Exception {
		...
		// test
		Skill nullObj = this.dao.getById(dObj.getId());
		assertNull(nullObj);
	}
	
	private Skill getLatest() throws Exception {
		List<Skill> dObjs = this.dao.getSortedListBySystemDomainId(PlanTestData.wellknown_institution, SortBy.id, false);
		return dObjs.get(0);
	}
