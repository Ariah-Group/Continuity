...
import org.kuali.continuity.admin.domain.ContactInfo.SortBy;

public class ContactInfoDAOTest extends AdminDAOTest {
	
	private GenericAdminDAOFactory factory;
	private GenericAdminDAO<ContactInfo> dao;
	
	@SuppressWarnings("unchecked")
	public void setFactory(GenericAdminDAOFactory factory) {
		this.factory = factory;
		this.dao = (GenericAdminDAO<ContactInfo>)
			factory.getGenericAdminDAO(ContactInfo.class);
	}
	...
	private void doTestCreate() throws Exception {
		ContactInfo dObj = new ContactInfo();
		dObj.setName("TEST");
	...
	private void doTestRead() throws Exception {
		// test 1
		ContactInfo dObj = this.getLatest();
		ContactInfo dSameObj = this.dao.getById(dObj.getId());
	...
	private void doTestUpdate() throws Exception {
		...
		// test
		ContactInfo modObj = this.dao.getById(dObj.getId());
	...
	private void doTestDelete() throws Exception {
		...
		// test
		ContactInfo nullObj = this.dao.getById(dObj.getId());
		assertNull(nullObj);
	}
	
	private ContactInfo getLatest() throws Exception {
		List<ContactInfo> dObjs = this.dao.getSortedListBySystemDomainId(PlanTestData.wellknown_institution, SortBy.id, false);
		return dObjs.get(0);
	}
