...
public class CentralApplicationDAOTest extends AdminDAOTest implements OrderingDef {
	
	private GenericAdminDAOFactory factory;
	private GenericAdminDAO<CentralApplication> dao;
	
	@SuppressWarnings("unchecked")
	public void setFactory(GenericAdminDAOFactory factory) {
		this.factory = factory;
		this.dao = (GenericAdminDAO<CentralApplication>)
			factory.getGenericAdminDAO(CentralApplication.class);
	}
	...
	public void testGetSortedLimitedList() throws Exception {
		...
		System.out.println("Limited descending sort by name");
	...
	private void doTestCreate() throws Exception {
		CentralApplication dObj = new CentralApplication();
		dObj.setName("TEST");
	...
	private void doTestRead() throws Exception {
		// test 1
		CentralApplication dObj = this.getLatest();
		CentralApplication dSameObj = this.dao.getById(dObj.getId());
	...
	private void doTestUpdate() throws Exception {
		...
		// test
		CentralApplication modObj = this.dao.getById(dObj.getId());
	...
	private void doTestDelete() throws Exception {
		...
		// test
		CentralApplication nullObj = this.dao.getById(dObj.getId());
		assertNull(nullObj);
	}
	
	private CentralApplication getLatest() throws Exception {
		List<CentralApplication> dObjs = this.dao.getSortedListBySystemDomainId(PlanTestData.wellknown_institution, SortBy.id, false);
		return dObjs.get(0);
	}
