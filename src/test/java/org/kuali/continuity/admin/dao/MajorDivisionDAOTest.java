...
import org.kuali.continuity.admin.domain.MajorDivision.SortBy;

public class MajorDivisionDAOTest extends AdminDAOTest {
	
	private GenericAdminDAOFactory factory;
	private GenericAdminDAO<MajorDivision> dao;
	
	@SuppressWarnings("unchecked")
	public void setFactory(GenericAdminDAOFactory factory) {
		this.factory = factory;
		this.dao = (GenericAdminDAO<MajorDivision>)
			factory.getGenericAdminDAO(MajorDivision.class);
	}
	...
	private void doTestCreate() throws Exception {
		MajorDivision dObj = new MajorDivision();
		dObj.setName("TEST");
	...
	private void doTestRead() throws Exception {
		MajorDivision dObj = this.getLatest();
		MajorDivision dSameObj = this.dao.getById(dObj.getId());
	...
	private void doTestUpdate() throws Exception {
		...
		// test
		MajorDivision modObj = this.dao.getById(dObj.getId());
	...
	private void doTestDelete() throws Exception {
		...
		// test
		MajorDivision nullObj = this.dao.getById(dObj.getId());
		assertNull(nullObj);
	}
	
	private MajorDivision getLatest() throws Exception {
		List<MajorDivision> dObjs = this.dao.getSortedListBySystemDomainId(PlanTestData.wellknown_institution, SortBy.id, false);
		return dObjs.get(0);
	}
