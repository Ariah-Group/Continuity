...
import org.kuali.continuity.admin.domain.Acronym.SortBy;

public class AcronymDAOTest extends AdminDAOTest {
	
	private GenericAdminDAOFactory factory;
	private GenericAdminDAO<Acronym> dao;
	
	@SuppressWarnings("unchecked")
	public void setFactory(GenericAdminDAOFactory factory) {
		this.factory = factory;
		this.dao = (GenericAdminDAO<Acronym>)
			factory.getGenericAdminDAO(Acronym.class);
	}
	...
	private void doTestCreate() throws Exception {
		Acronym dObj = new Acronym();
		dObj.setFullName("Full Testing");
		dObj.setName("TEST");
	...
	private void doTestRead() throws Exception {
		Acronym dObj = this.getLatest();
		Acronym dSameObj = this.dao.getById(dObj.getId());
		assertNotNull(dSameObj);
	}
	
	private void doTestUpdate() throws Exception {
		Acronym dObj = this.getLatest();
		String modFullName = dObj.getFullName() + "-Modified";
		dObj.setFullName(modFullName);
		
		// update
		this.dao.update(dObj);
		
		// test
		Acronym modObj = this.dao.getById(dObj.getId());
		assertEquals(modFullName, modObj.getFullName());
	}
	...
	private void doTestDelete() throws Exception {
		...
		// test
		Acronym nullObj = this.dao.getById(dObj.getId());
		assertNull(nullObj);
	}
	
	private Acronym getLatest() throws Exception {
		List<Acronym> dObjs = this.dao.getSortedListBySystemDomainId(PlanTestData.wellknown_institution, SortBy.id, false);
		return dObjs.get(0);
	}
