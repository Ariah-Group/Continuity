...
import org.kuali.continuity.admin.domain.Utility.SortBy;

public class UtilityDAOTest extends AdminDAOTest {
	
	private GenericAdminDAOFactory factory;
	private GenericAdminDAO<Utility> dao;
	
	@SuppressWarnings("unchecked")
	public void setFactory(GenericAdminDAOFactory factory) {
		this.factory = factory;
		this.dao = (GenericAdminDAO<Utility>)
			factory.getGenericAdminDAO(Utility.class);
	}
	...
	private void doTestRead() throws Exception {
		// test 1
		Utility dObj = this.getLatest();
		Utility dSameObj = this.dao.getById(dObj.getId());
		...
		// test 3
		dObj = this.dao.getBySystemDomainIdAndName(PlanTestData.wellknown_institution, "Vacuum");
		assertNotNull(dObj);
	...
	private void doTestUpdate() throws Exception {
		...
		// test
		Utility modObj = this.dao.getById(dObj.getId());
	...
	private void doTestDelete() throws Exception {
		...
		// test
		Utility nullObj = this.dao.getById(dObj.getId());
		assertNull(nullObj);
	}
	
	private Utility getLatest() throws Exception {
		List<Utility> dObjs = this.dao.getSortedListBySystemDomainId(PlanTestData.wellknown_institution, SortBy.id, false);
		return dObjs.get(0);
	}
