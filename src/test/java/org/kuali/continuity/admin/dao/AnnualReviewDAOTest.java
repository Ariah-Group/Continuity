...
import org.kuali.continuity.admin.domain.AnnualReview.SortBy;

public class AnnualReviewDAOTest extends AdminDAOTest {
	
	private GenericAdminDAOFactory factory;
	private GenericAdminDAO<AnnualReview> dao;
	
	@SuppressWarnings("unchecked")
	public void setFactory(GenericAdminDAOFactory factory) {
		this.factory = factory;
		this.dao = (GenericAdminDAO<AnnualReview>)
			factory.getGenericAdminDAO(AnnualReview.class);
	}
	...
	private void doTestCreate() throws Exception {
		AnnualReview dObj = new AnnualReview();
		dObj.setName("TEST");
	...
	private void doTestRead() throws Exception {
		// test 1
		AnnualReview dObj = this.getLatest();
		AnnualReview dSameObj = this.dao.getById(dObj.getId());
	...
	private void doTestUpdate() throws Exception {
		...
		// test
		AnnualReview modObj = this.dao.getById(dObj.getId());
	...
	private void doTestDelete() throws Exception {
		...
		// test
		AnnualReview nullObj = this.dao.getById(dObj.getId());
		assertNull(nullObj);
	}
	
	private AnnualReview getLatest() throws Exception {
		List<AnnualReview> dObjs = this.dao.getSortedListBySystemDomainId(PlanTestData.wellknown_institution, SortBy.id, false);
		return dObjs.get(0);
	}
