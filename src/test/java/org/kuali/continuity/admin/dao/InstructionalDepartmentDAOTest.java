...
import org.kuali.continuity.admin.domain.InstructionalDepartment.SortBy;

public class InstructionalDepartmentDAOTest extends AdminDAOTest {
	
	private GenericAdminDAOFactory factory;
	private GenericAdminDAO<InstructionalDepartment> dao;
	
	@SuppressWarnings("unchecked")
	public void setFactory(GenericAdminDAOFactory factory) {
		this.factory = factory;
		this.dao = (GenericAdminDAO<InstructionalDepartment>)
			factory.getGenericAdminDAO(InstructionalDepartment.class);
	}
	...
	private void doTestCreate() throws Exception {
		InstructionalDepartment dObj = new InstructionalDepartment();
		dObj.setName("TEST");
	...
	private void doTestRead() throws Exception {
		InstructionalDepartment dObj = this.getLatest();
		InstructionalDepartment dSameObj = this.dao.getById(dObj.getId());
	...
	private void doTestUpdate() throws Exception {
		...
		// test
		InstructionalDepartment modObj = this.dao.getById(dObj.getId());
	...
	private void doTestDelete() throws Exception {
		...
		// test
		InstructionalDepartment nullObj = this.dao.getById(dObj.getId());
		assertNull(nullObj);
	}
	
	private InstructionalDepartment getLatest() throws Exception {
		List<InstructionalDepartment> dObjs = this.dao.getSortedListBySystemDomainId(PlanTestData.wellknown_institution, SortBy.id, false);
		return dObjs.get(0);
	}
