...
@Table(name="FAQ")
@NamedQueries({
	@NamedQuery(name="FAQ.list",
		query="SELECT dObj FROM FAQ dObj WHERE dObj.systemDomainId = :systemDomainId ORDER BY dObj.orderNo"),
	@NamedQuery(name="FAQ.size",
		query="SELECT COUNT(dObj) FROM FAQ dObj WHERE dObj.systemDomainId = :systemDomainId")
})
@SuppressWarnings("serial")
public class FAQ extends BaseAdminOrderedDomainObject implements OrderedType {
	
	private String answer;

	public enum SortBy implements SortByType {
		id, orderNo, question, answer;
