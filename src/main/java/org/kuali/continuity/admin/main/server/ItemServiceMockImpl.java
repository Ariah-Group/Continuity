...
public class ItemServiceMockImpl  implements ItemService, ListService {
	...
	static List mocklist = new ArrayList() {
		 {
			Item r1 = createMockItem(0,"an item named a\n<br>and so on",1);
	 ...
	 protected static Item createMockItem(int id, String name, int order) {
			Item ret = new Item();
			ret.setId(id);
			ret.setName(name);
			ret.setOrder(""+order);
